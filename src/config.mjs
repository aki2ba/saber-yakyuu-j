...
export const TUNING_DEFAULT = {
  ...
  hrScale: 0.985, // 本塁打産出スケール（門番: hrPerTeam/HR王）。⚠️HRは閾値のため感度大（D1-1: HR飛距離モデル明示化・集中は無効のため baseline 値を維持）。D2: 球場個性の凸性でHR/teamが僅増するがSDを抑えた結果 baseline を維持（再較正で hrScale 据え置き＝120帯）
  ...
  pitch: {
    ...
    // --- スイング時の空振り ---
    whiffZoneBase: 0.185, // ゾーン内スイングの空振り基準（B1較正: 0.095→0.175。0.9.1: →0.185。
    //   帯再配分で空振りの多いchase swingが減った分をゾーン内で補い Contact%[75,81] を維持）
    whiffBorderBase: 0.32, // ボーダー帯スイングの空振り基準（B1較正: 0.16→0.27。0.9.1: →0.32・同上）
    whiffOBase: 0.44, // ボール球スイング（chase）の空振り基準（B1較正: 0.26→0.38。0.9.1: →0.44・同上）
    whiffPitchW: 0.006, // 球種whiff(50中心)→空振り増
    whiffContactW: 0.006, // 打者contact(50中心)→空振り減
    whiffAptW: 0.004, // 対該当クラス適性(50中心)→空振り減
    whiffTwoStrikeW: 0.215, // 2ストライクの空振り率減（当てにいく短縮スイング。ppa↑・K抑制・B1較正: 0→0.18。
    //   0.9.1: →0.215。whiff基準の引上げでK%が上限を超えないよう2ストライク側で相殺＝K[18,20.5]を維持）
    // --- 接触時: ファウル vs インプレー ---
    foulBase: 0.47, // 接触のうちファウルになる基準
    foulTwoStrikeW: 0.18, // 2ストライクでファウル率増（粘り＝カット・B1較正: 0.08→0.18で投球数/PA↑）
    // --- 見逃し時: ボーダー帯の捕手フレーミング判定 ---
    borderCsBase: 0.183, // ボーダー帯見逃しのストライク獲得基準（framing50=リーグ中立時。0.9.1: 0.24→0.183。
    ...
    hbpPerClearBall: 0.0104, // 明確ボール1球あたりのHBP率（0.9.1: 0.007→0.0104。明確ボール36.6%→~24.7%の
    //   帯再配分でHBP機会が0.675倍になったため率で相殺＝HBP/PA~0.9%を維持）
