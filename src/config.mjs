...
export const TUNING_DEFAULT = {
  ...
  pitch: {
    ...
    zoneBase: 0.4035, // 基準ゾーン内率（control50・even count）※Zone%の主ノブ（B1較正: 0.42→0.432。
    ...
    // --- スイング時の空振り ---
    whiffZoneBase: 0.177, // ゾーン内スイングの空振り基準（B1較正: 0.095→0.175。0.9.1: →0.185。
  ...
  bb: {
    // EV(打球速度 km/h)
    evBase: 139.65, // 平均打球速度の中心 ※D1-2: 同利きEV罰則強化(-1.2→-2.5)でリーグ平均EVが下がった分を全体で再センタリング（139→139.55・同利き相対差は不変）。
