...
export const TUNING_DEFAULT = {
  ...
  // 塁打決定の守備隊形依存化（realism_r1_baserunning_spec §6）。
  // decideBases/expectedBases が「落下距離だけ」で無条件2Bにしていた穴（外野手の目の前の
  // ポトリが必ずツーベースになる）を、落下点と最寄り外野手の位置関係から創発させる。
  run2b: {
    frontDropRadiusM: 15, // 外野手の目の前/手前(beyond<=0)のポトリはこの半径以内なら単打とみなす
    ...
    gapPivotM: 14, // ギャップ/ライン際球の2B化ロジスティック基準距離（最寄り外野手からの距離）
    gapWidthM: 5, // 〃 の幅
