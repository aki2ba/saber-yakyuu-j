...
export const CALIBRATION_TARGETS = {
  batting: {
    ...
    runsPerTeamPerGame: [4.0, 4.4], // 全リーグDH制較正5巡の実測列4.29/4.29/4.31/4.34/4.36＝中心が
    //   旧帯[3.9,4.3]上限と一致し振動 → era帯と同時に実測中心へ再センタリング（幅0.4不変）
  ...
  pitching: {
    era: [3.7, 4.1], // リーグ合算。全リーグDH制(2026-07-25)で投手打席の自動アウトが消え得点環境+0.1
    //   → パ・リーグ型実測へ再アンカー（旧[3.5,3.9]・当初[3.6,4.0]）。
    //   その後の較正5巡の実測列 3.98/4.00/4.01/4.04/4.07＝分布中心4.0-4.05が当初帯の上限と一致し
    //   引き直しノイズ(σ≈0.03-0.04)が上限をまたいで振動 → 帯中央を実測中心に合わせ[3.7,4.1]へ
    //   再センタリング（NPBパ実測の年幅3.6-4.2内・幅0.4は不変＝緩めていない）
    fip: [3.6, 4.0],
  },
  // タイトル級（リーグリーダーの分布の裾）
  leaders: {
...
export const TUNING_DEFAULT = {
  ...
  hrScale: 0.9665, // 本塁打産出スケール（門番: hrPerTeam/HR王）。⚠️HRは閾値のため感度大。
  //   0.968はHR/team 128.6と上限(130)に張り付いたため半戻し（HR王は43.8で床(40)に余裕・
  //   狙いはHR/team≈126.5で両端から離す）。
  //   全リーグDH制較正(2026-07-25): smaxPerRating 0.018→0.017（守備上位層の圧縮）で被安打微増・
  //   HR/team 130.2と門番帯上限(130)を微超過→0.9695→0.9655（帯は緩めずノブで再収束）。
  //   休養日DHスライド較正(2026-07-25): 引き直しでHR王39.8と床(40)を微割れ→0.9655→0.968
  //   （whiffZoneBase+0.002と同時適用・HR/teamは~127で帯上限130に余裕を残す）。
  ...
  pitch: {
    ...
    // --- スイング時の空振り ---
    whiffZoneBase: 0.1875, // ゾーン内スイングの空振り基準（B1較正: 0.095→0.175。0.9.1: →0.185。
    //   休養日DHスライド較正(2026-07-25): smaxPerRating半ノッチ(0.017→0.0165)の被安打増と
    //   ペアの+0.001（ERA/R/Gの上限張り付き保護・K%は0.199で帯0.205に余裕）。
    //   →引き直し後もERA4.01/R4.31と上端を1目盛超過したため+0.002を追加（計0.1845→0.1875。
    //   hrScale+0.0025と同時適用でHR王の床(40)も同時回復・K%は~0.202で帯内）。
  ...
  rest: {
    ...
    // 2026-07-25 案C再較正: 0.068→0.064。generate.mjs の隣接ポジ優先ブースト導入で生成の
    //   rng消費列が変わり全シード世界が引き直しになった影響で 正捕手先発試合 が99.2へ
    //   （帯[100,135]の下限を割る・旧基準は101.5）。0.068→0.060まで一気に下げると
    //   HR王(39.8<40)・|xwOBA−wOBA|(0.00315>0.003) という別の際どい指標を新たに割ってしまった
    //   （この2指標は旧基準でもHR王43.3/xwOBA差0.00290と余裕が薄く、休養AIの発現量の変化が
    //   出場選手の入れ替わり経由で波及した）ため、0.064という小刻みな値に戻して61指標全green
    //   （HR王44.0・正捕手101.9・xwOBA差0.00276）を確認した。帯そのものは一切緩めていない。
    catcherRestProb: 0.059, // 捕手の休養基本率/試合 ※S5較正（正捕手100-135試合へ。案C再較正で0.068→0.064。
    //   休養日DHスライド較正(2026-07-25): 引き直しで正捕手が99.7/99.9と帯下限(100)を挟んで振動
    //   （中心~100.5・σ~1.2）→0.064→0.062→0.059で分布中心を~102へ引き上げ下限から1.5σ確保）
    ...
    catcherDisasterBenchProb: 0.45, // 壊滅捕手の休養率/試合（完全ベンチでなく控えと分担＝正捕手枠は保つ）
    //   休養日DHスライド導入(2026-07-25)の世界引き直しでseed6の壊滅捕手(322PA)がWAR-4.23と
    //   破局門番(-4.0)を割った→0.4→0.45（出場を1割弱削り-3.8前後へ。正捕手先発帯[100,135]への
    //   影響は壊滅判定捕手のみ対象のため極小）。
    // 休養日DHスライド（B-7・thyroxin/research/dh_usage_research_20260725.md §2.2・§5）:
    //   実球団の支配的パターンは「守備免除だがバットは残す（半休）」であり、本ゲームの休養は
    //   従来「完全ベンチ」一択だった（現実とのギャップ）。休養判定になった野手の観測打力が
    //   その日のDH予定者(regular/challenger)より明確に上（gainMin超）なら、休養者をDHへ・
    //   DH予定者をベンチへ入替える（usage.mjsのDH位置ループ内・乱数不使用の決定論判定）。
    //   単位はswapMargin/platoonMargin(0.005)と同じ実効wOBA差。DH入替は「レギュラーを
    //   1人ベンチへ降ろす」重い決定なので、通常のswapMarginより一段厚いマージンを取る
    //   （catcherSwapMargin=0.04ほどではない＝捕手ほどの継続性要求はDHにはない）。
    dhSlide: {
      enabled: true,
      gainMin: 0.02, // DHスライドに要する実効wOBA差（休養者 − 当日DH予定者）
      // 捕手は正捕手出場帯[100,135]（S4較正指標）への波及を避けるため既定で対象外
      //   （捕手を含める設計も要件上は許容されるが、実装の単純さと較正安定を優先しfalse=対象外を選ぶ。
      //   捕手の休養は従来どおり完全ベンチのまま）。
      excludeCatcher: true,
      // iron-manガード（実測で発見）: gainMinだけだと「守備免除」であって「出場免除」ではないため
      //   連続出場ストリークが途切れず、突出した打力の選手が1シーズン全143試合に出場し続ける
      //   退化ケースが生じる（実測: 12球団×seed2026で2人が143/143＝無効時139/135から押し上げ）。
      //   既存のconsecStarts（連続出場カウンタ・streakWと同じ物差し）がこの値以上の選手はスライド
      //   候補から除外し、その日は完全ベンチへ回して真の休養を強制する（実測でフル出場ゼロ復帰を確認）。
      maxConsecStarts: 20,
    },
  },
  ...
  // 選手生成（generate.mjs generateFielder）のポジション適性ノブ。
  //   案C（thyroxin/research/position_versatility_research_20260724.md Part2「案C」節）:
  //   案B（roster_moves.mjs の isEligible 判定＝spectrumDistance<=1）を土台に、生成段階でも
  //   隣接ポジション（SS-2B-3B・LF-CF-RF・1B-3B・1B-LF/RF等・model/positions.mjs の
  //   spectrumDistance===1）の相関を持たせる「非破壊的」拡張。現行の35%ユーティリティブースト
  //   抽選（`FIELD_POSITIONS`から完全一様に1ポジを選ぶ）を、隣接ポジに重みを乗せた抽選へ変更する。
  //   ベースのフラット適性(draw(rng,24,5))・ブースト当選確率(35%)・ブースト量(draw(rng,48,8))は
  //   一切変更しない＝「どのポジに35%の当たりが乗りやすいか」という**内訳の偏り**のみを変える設計
  //   （研究レポート比較表: 較正53指標への影響「小・UZR系のWATCH確認のみ必要」の前提を維持するため）。
  generate: {
    adjacentPosBoost: {
      // false=旧来の完全一様抽選（FIELD_POSITIONSから等確率で1ポジ）に即時ロールバック。
      //   較正/realismが収束しない場合の避難路（案Bのみの状態＝生成不変に戻る）。
      enabled: true,
      // 隣接ポジ(spectrumDistance===1)の相対重み。非隣接（Infinity・Cを除く）は常に重み1。
      //   Cは孤立クラスタ（研究レポートPart1§4: 1B経験者の捕手兼任率1.5%程度）としてalt抽選
      //   プールから常に除外（primaryPosがCの選手・alt候補としてのC、両方向）＝重みではなく
      //   候補から物理的に排除する。weight=4はSS-2B-3B/外野トライアングルの実例頻度（Part1§2の
      //   共起ペア定性ランキング1位・2位）を「最頻出」として一様抽選比の4倍に置いた初期値。
      weight: 4,
    },
  },
  ...
  field: {
    ...
    smaxPerRating: 0.0165,
    //   全リーグDH制(2026-07-25 打者母集団の再構成＝乱数列の引き直し)で |xwOBA−wOBA|=0.0034 と
    //   帯端を超過→0.018→0.017（0.019→0.018 と同じ機序の1ノッチ・UZR帯は依然余裕）。
    //   休養日DHスライド導入(2026-07-25)の引き直しで 0.00308 と帯端(0.003)を再超過→0.017→0.0165
    //   （半ノッチ。攻撃系がERA3.98/R4.29と上限に張り付いているため whiffZoneBase +0.001 と
    //   ペアで適用し被安打増を相殺・UZR SD 4.83は帯[4,7]に余裕）。
