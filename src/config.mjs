...
export const TUNING_DEFAULT = {
  ...
  rest: {
    ...
    catcherRestProb: 0.070, // 捕手の休養基本率/試合 ※S5較正（正捕手100-135試合へ）
  ...
  injury: {
    ...
    // ★R3: 捕手の割増を撤廃（0.050→0）。「捕手は壊れる」は定量データに支持されない:
    //   捕手の負傷率 2.75/1000 Athlete-Exposure は他ポジションより **低い**（Guy 2015・MLB 2001-10・
    //   PubMed 26320222）。Carr 2022 でも捕手は負傷burdenが最低(11.0%)。ただし膝・頭頸部の負傷は
    //   重症化しやすい → **頻度でなく重症度で差をつける**（下の majorCatcher）。
    catcher: 0.0,
    ...
    cap: 0.55, // ハザードの上限
    // ★R3 重症率の再較正: 現実は「シーズン絶望級＝全故障の 8-11%」（Esquivel 2019・MLB HITS
    //   n=51,548・確度高）。旧値 0.22 は過大だった。
    majorGivenInjury: 0.07, // 故障したうち重症になる基本割合
    majorHistBonus: 0.05, // 故障歴があると重症化しやすい
    majorPitcherLegacy: 0.15, // 投手が過去に大怪我していると更に重症化しやすい
    // ★R3: 投手・捕手は「頻度」でなく「重症度」で差がつく（＝離脱が長い）。
    //   投手の故障部位は肩64日/肘59日、野手は肉離れ40日/その他30-35日（データスタジアム 2017）。
    //   man-days lost 比は投手:野手 ≈ 1.3-1.8:1 だが、**件数比は 1.0-1.3:1** に過ぎない
    //   （NPB実測 2023 1.10:1 / 2024 1.18:1）。捕手も膝・頭頸部は重い。
    majorPitcher: 0.06, // 投手は重症化しやすい（肩肘）
    majorCatcher: 0.05, // 捕手は頻度は低いが重症化しやすい（膝・頭頸部）
    ...
    // ★R3: 試合中の故障発生（sim/injury.mjs の exposureProb / sim/game.mjs が消費）。
    //   旧実装は「オフに1選手1回ロール→翌年の開幕IL」で、シーズン中に誰も壊れなかった
    //   （＝1年目は故障者ゼロ／一軍二軍の入替が NPB の 1/5 しか湧かない主因）。
    //   ここでは「打席で身体を使った選手」の露出イベントごとに引く。個体差（年齢/球速/捕手/
    //   故障歴）は上の injuryHazard をそのまま倍率として使う（ハザードの形は1箇所に集約）。
    inSeason: {
      enabled: true,
      refHazard: 0.06, // 倍率の正規化基準（injuryHazard/refHazard=1 の選手が下の base 確率）
      // 露出ごとの確率（realism-refs の NPB_INJURY に対して calibrate/realism で較正した値）。
      //   現実の「試合中の負傷の契機」はほぼ均等（投球23%/打撃24%/守備23%/走塁22%・Esquivel 2019）。
      //   走塁は打者の露出(perPA)に含める。投手の perBF は登板日以外のブルペン/投げ込みの消耗も
      //   代理しており（現実の故障の37%は試合外・うち29.8%が throwing）、投球回数に比例させる。
      perPA: 0.00080, // 打者: 1打席（スイング・全力疾走）
      perBF: 0.00075, // 投手: 対戦打者1人（肩肘の消耗）※球速・故障歴で倍率が乗る
      perFieldPlay: 0.00100, // 野手: 自分が処理した打球1つ（守備機会）
      perCatcherPA: 0.00004, // 捕手: 守備側1打席（ファウルチップ/ブロッキング。頻度は割増しない＝上記）
      // 離脱の単位換算: gamesLost（試合数）→ 日程の day 数。日程には全休日があるので day > game。
      //   シーズン開始時に schedule から実測して上書きされる（この値はフォールバック）。
      daysPerGame: 1.2,
    },
  },
