...
export const TUNING_DEFAULT = {
  ...
  injury: {
    ...
    catcher: 0.0,
    // ★R4 再発リスクの上限（故障歴スパイラルの遮断）:
    //   旧実装は故障歴1件ごとにハザードを**無制限に累積**していた。R3で故障が試合中に発生する
    //   ようになり、1選手が1年に複数回壊れうるようになったため、キャリアを通じて履歴が積み上がり
    //   **20年後のリーグは全員が壊れやすい**状態になっていた（実測: 規定到達/球団が 5.8→3.9 と
    //   較正帯[5,9]を割り、防御率も 3.98→4.16 へ悪化）。現実の「前回の故障が最大の予測因子」は
    //   事実だが、10年前の肉離れが今日のリスクを上げ続けることはない。→ 合計に上限を設ける。
    recurMinor: 0.008, // 軽症の故障歴1件ごとの再発リスク加算
    recurMajor: 0.022, // 重症の故障歴1件ごとの再発リスク加算（最大リスク）
    recurCap: 0.040, // 故障歴による加算の合計上限（＝故障がちな選手でも2倍弱に留める）
    pitcherMajorLegacy: 0.030, // 投手が過去に大怪我＝以後の恒常的な将来リスク（非対称。上限の外側）
    cap: 0.55, // ハザードの上限
