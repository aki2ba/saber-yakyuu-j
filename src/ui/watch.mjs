...
// F1コース図v2（phaseF_spec・UIのみ）:
//   - 投手目線化: TVセンターカメラと同じ向き（一塁側=画面右・三塁側=画面左）。watchPitchXY の
//     x を最後に左右反転するだけ（決定論ハッシュ・帯データは不変＝座標変換のみ）。
//   - 打者の影: ゾーン脇に半透明シルエット（右打者=画面左/左打者=画面右・スイッチは effectiveBats）。
//   - 利き腕表示: 対戦カードに「右打/左打/両打(実効側)」「右投/左投」（bats/throws は公開情報）。
//   - 球種マーカー: 形＝球種（●▲◆▼■⬢）／色＝判定（pc-*維持）／番号=右肩。凡例2段化。
// 設計原則:
...
// ============================================================================
import { playerBatting, playerPitching, hashSeed, effectiveBats } from '../engine.mjs';
...
// F1: 球種→マーカー形状（形＝球種／色＝判定／番号=右肩）。model/positions.mjs の PITCH_TYPES 全種をカバー
// （fastball/sinker/cutter/slider/curve/changeup/fork）。未知の球種は hex にフォールバック。
const WATCH_PITCH_SHAPE = {
  fastball: 'circle', // ●ストレート
  slider: 'triup', // ▲スライダー
  curve: 'diamond', // ◆カーブ
  fork: 'tridown', // ▼フォーク
  changeup: 'square', // ■チェンジアップ
  sinker: 'hex', // ⬢シンカー/カット系（その他）
  cutter: 'hex',
};
// 凡例1段目「形＝球種」の表記（全球種を6形状に集約して掲示）
const WATCH_SHAPE_LEGEND = [['●', 'ストレート'], ['▲', 'スライダー'], ['◆', 'カーブ'], ['▼', 'フォーク'], ['■', 'チェンジ'], ['⬢', 'シンカー/カット']];
 ...
 * F1投手目線化: 決定論ハッシュはそのままに、最後に x を左右反転（座標変換のみ）。
 *  TVセンターカメラと同じ向き＝一塁側が画面右・三塁側が画面左（ゾーンは水平中央なので枠内は枠内のまま）。
 */
function watchPitchXY(rec, abIdx, n, band, swung = false) {
  ...
  if (band === 0) return [W - (zx + 6 + key1 * (zw - 12)), zy + 7 + key2 * (zh - 14)]; // ゾーン枠内（投手目線へ反転）
  ...
  return [W - x, y]; // 投手目線＝捕手視点から左右反転
}

/** コース図カラム（見出し＋ゾーンSVG＋凡例2段: 形＝球種／色＝判定）。F1で投手目線化。 */
function watchZoneCol(v, u, w) {
  const { el } = u;
  const col = el('div', { class: 'zonecol' });
  col.append(el('div', { class: 'curabhead' }, 'コース（投手目線）'));
  col.append(watchZonePlot(v, u, w));
  // 凡例1段目「形＝球種」（実際の速報アプリの再現・全球種を6形状に集約）
  col.append(el('div', {
    class: 'zonelegend legshape',
    title: 'マーカーの形が球種を表します（●ストレート ▲スライダー ◆カーブ ▼フォーク ■チェンジアップ ⬢シンカー/カット系）。',
  }, [
    el('span', { class: 'leglabel' }, '形＝球種:'),
    ...WATCH_SHAPE_LEGEND.map(([s, name]) => el('span', {}, s + name)),
  ]));
  // 凡例2段目「色＝判定」（現行の pc-* 統一色）
  col.append(el('div', {
    class: 'zonelegend legcolor',
    title: '高さ・コースの帯（ゾーン内／際／明確ボール）は投球の実データ。帯の中の細かい位置は決定論ハッシュによる演出です（同じ試合は何度見ても同じ図）。',
  }, [
    el('span', { class: 'leglabel' }, '色＝判定:'),
...
/**
 * ストライクゾーンプロット本体（F1・投手目線）: ゾーン枠＋九分割目安線＋打者の影＋
 * 現打席の投球マーカー（形＝球種／色＝判定／番号=右肩）。
 */
function watchZonePlot(v, u, w) {
  ...
  watchBatterShadow(svg, v, u); // 打者の影（ゾーン枠より奥＝先に描く）
  ...
  if (ab) {
    for (const p of ab.pitches) {
      ...
      const cls = watchCallCls(p.call);
      svg.append(watchPitchMarker(u, x, y, p.type, 'pdot ' + cls)); // 形＝球種・fill=判定色
      // 球番号①②…は形の右肩に小さく
      svg.append(svgText({ x: x + 5.2, y: y - 3.2, class: 'pnum ' + cls, 'font-size': '7.5', 'font-weight': '700' }, watchCircledNum(p.n)));
    }
  }
  return svg;
}

/** F1: 球種→SVGマーカー（circle/polygon/rect・fill は判定色クラス pc-* に委譲）。 */
function watchPitchMarker(u, x, y, type, cls) {
  const { svgEl } = u;
  const kind = WATCH_PITCH_SHAPE[type] || 'hex';
  if (kind === 'circle') return svgEl('circle', { cx: x.toFixed(1), cy: y.toFixed(1), r: 4.8, class: cls });
  if (kind === 'square') return svgEl('rect', { x: (x - 4.2).toFixed(1), y: (y - 4.2).toFixed(1), width: 8.4, height: 8.4, class: cls });
  let pts;
  if (kind === 'triup') pts = [[0, -5.6], [5, 4.2], [-5, 4.2]];
  else if (kind === 'tridown') pts = [[0, 5.6], [5, -4.2], [-5, -4.2]];
  else if (kind === 'diamond') pts = [[0, -6], [5, 0], [0, 6], [-5, 0]];
  else pts = Array.from({ length: 6 }, (_, i) => { // hex（⬢・フラットトップ）
    const a = Math.PI / 6 + (i * Math.PI) / 3;
    return [5.4 * Math.cos(a), 5.4 * Math.sin(a)];
  });
  return svgEl('polygon', { points: pts.map(([dx, dy]) => `${(x + dx).toFixed(1)},${(y + dy).toFixed(1)}`).join(' '), class: cls });
}

/**
 * F1: 打者の影（半透明シルエット・簡易SVG人型: 頭円＋胴＋脚＋バット線）＋「右打/左打」ラベル。
 * 投手目線＝右打者は三塁側（画面左）・左打者は一塁側（画面右）。
 * スイッチヒッターは対戦投手との実効打席（engine の effectiveBats・公開情報）で側を決める。
 */
function watchBatterShadow(svg, v, u) {
  const { svgEl, svgText, state } = u;
  const { W } = WATCH_ZP;
  const bat = v.batterId ? state.byId.get(v.batterId) : null;
  if (!bat) return;
  const pit = v.curPitcherId ? state.byId.get(v.curPitcherId) : null;
  const side = pit ? effectiveBats(bat, pit) : (bat.bats === 'S' ? 'L' : bat.bats); // 'R'|'L'（実効打席）
  const tx = side === 'R' ? 26 : W - 26; // 右打=画面左（三塁側）／左打=画面右（一塁側）
  // ローカル座標は「画面左の打者（バットは+x=ゾーン向き）」で定義し、左打者は scale(-1,1) で鏡映
  const g = svgEl('g', { class: 'batshadow', transform: `translate(${tx} 0)${side === 'L' ? ' scale(-1 1)' : ''}` });
  g.append(svgEl('circle', { cx: 0, cy: 58, r: 7 })); // 頭
  g.append(svgEl('rect', { x: -7, y: 67, width: 14, height: 36, rx: 6 })); // 胴
  g.append(svgEl('line', { x1: -3.5, y1: 103, x2: -6.5, y2: 140, 'stroke-width': 5.5 })); // 脚
  g.append(svgEl('line', { x1: 3.5, y1: 103, x2: 6.5, y2: 140, 'stroke-width': 5.5 }));
  g.append(svgEl('line', { x1: 5, y1: 74, x2: 20, y2: 46, 'stroke-width': 3.2 })); // バット（構え）
  svg.append(g);
  // ラベルは鏡映グループの外（文字を反転させない）・影の頭側に
  svg.append(svgText({ x: tx, y: 42, class: 'batlabel', 'font-size': '9', 'text-anchor': 'middle' }, side === 'R' ? '右打' : '左打'));
}
