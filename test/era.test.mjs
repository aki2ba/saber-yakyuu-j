...
test('D3: 時代の揺れが得点環境を動かす — 打高(evBase+) > 中立 > 投高(evBase−)', () => {
  ...
  // 2026-07-25 案C（generate.mjs の隣接ポジ優先ブースト・positionProf生成のrng消費列変更）で
  //   全シード世界が引き直しになった影響で、旧3シード[42,7,101]平均だと打高4.220<中立4.238と
  //   僅差反転（誤差±0.02R/G＝ノイズ床）が発生した。8シードへ拡張すると
  //   打高4.30/4.38 > 中立4.25 > 投高4.02/4.11 と2種の独立シード集合で頑健に単調性が戻る
  //   ことを確認済み（このテストが検出したいのは evBaseDelta の実効果であり、3シードは
  //   そもそも本文コメントが警告する「反転しうる」ノイズ床ぎりぎりの標本数だった）。
  const seeds = [42, 7, 101, 17, 55, 88, 3, 9];
  const rpg = (delta) => {
    let rs = 0;
    let g = 0;
