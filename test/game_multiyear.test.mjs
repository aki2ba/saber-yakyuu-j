...
test('多年運用: リーグ得点環境が各年ずっと NPB 帯付近に留まる（単調インフレしない・§11.3）', () => {
  for (let y = 0; y < YEARS; y++) {
    ...
    // 全リーグDH制(2026-07-25): 投手打席の自動アウト消滅でERAの水準が全年+0.1底上げ（1年目帯の
    //   再アンカー3.9→4.0と同じ幅）→ 上限4.78→4.9（実測 yi19=4.80）。多年再較正時にまとめて締め直すこと。
    assert.ok(a.era >= 3.3 && a.era <= 4.9, `yi${y}: 平均ERA=${a.era.toFixed(2)} が帯[3.3,4.9]内`);
