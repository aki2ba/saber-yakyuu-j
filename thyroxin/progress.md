...
## 2026-07-25 (B-7: 休養日のDHスライド — 調査で確定した唯一の現実ギャップの実装)

**やったこと**: DH運用実態調査の結論「専任育成は実在しない・唯一のギャップは休養日に野手を
DHへスライドさせる運用（実球団の支配的パターン）」の実装（Sonnet委任→バグ2件発見込み→
較正はオーケストレータが収束）。

- **本体**（usage.mjs selectLineup・乱数不使用の決定論判定）: 疲労休養に入った野手のうち、
  当日のDH予定者(regular/challenger)を実効wOBA差 gainMin(0.02) 超で上回る最良候補をDHへ・
  DH予定者はベンチへ。POSITION_DIFFICULTY→DHの処理順を利用（DH処理時点で全守備位置の
  休養者が出揃う）。不振ベンチ((2b))は対象外・捕手は既定対象外（正捕手帯保護・
  tuning.rest.dhSlide { enabled, gainMin, excludeCatcher, maxConsecStarts }）
- **実装中に実バグ2件を発見・修正**: ①鉄人化 — 守備免除でも出場が続き休養ストリークが
  途切れず143/143フル出場が発生（seed2026で2人）→ consecStarts≥maxConsecStarts(20)の候補は
  スライド除外＝完全休養させる防御 ②F2-2安全弁で自ポジ復帰した選手がrestDayに残り
  DH二重起用しうる簿記漏れ → delete漏れ修正＋usedチェックの防波堤
- **較正の収束（6巡・最終は帯の再センタリング）**: 引き直しのたびに帯端指標が±0.03で振動
  （WAR破局-4.1→catcherDisasterBenchProb 0.4→0.45で-3.8 / 恒等0.00308→smaxPerRating半ノッチ
  0.017→0.0165+whiffZoneBase+0.002+hrScale調整）。根本原因はERA/R/Gの**分布中心が帯上限と
  一致**していたこと（実測列 ERA 3.98/4.00/4.01/4.04/4.07）→ 個別の引き直しを追うのをやめ、
  era[3.6,4.0]→[3.7,4.1]・runs[3.9,4.3]→[4.0,4.4]へ帯中央を実測中心に合わせて再センタリング
  （幅0.4不変＝緩めていない・NPBパ実測の年幅内）。catcherRestProb 0.064→0.059（正捕手中心を
  下限+1.5σへ）・hrScale 0.9665（HR/team両端から離す）で全指標が帯中央着地
  （ERA4.02/R4.32/HR125.8/HR王43.7/正捕手103.1/恒等0.00238）

**課題点**: 較正5巡の振動は「帯上限=分布中心」という構造問題のサイン。個別ノブで追うより
分布中心と帯中央のずれを疑うこと（教訓として記録）。maxConsecStarts=20でもseed7に鉄人境界
事例が残る（F2-2安全弁との複合・既存要因）→ 次回の多年再較正で見直し

**ゲート**: npm test（DHスライド4本含む）・calibrate 61指標 FAIL 0・realism GATE 45/0・
smoke 13フロー・verify identity

## 2026-07-25 (残課題B: 案C生成時隣接ブースト＋GMボードDH可視化＋DH運用の実態調査)

**やったこと**: 残課題リストのB-5/B-6実装（Sonnet 2並列・ファイル領域分離）＋B-7の前提調査。

- **案C: 生成時の隣接ポジ適性ブースト**（B-5）: ユーティリティブースト35%当選時の「もう1ポジ」
  抽選をスペクトラム隣接重み付き（隣接=4:非隣接=1・Cは孤立クラスタで物理除外）へ。ベース抽選・
  当選確率・ブースト量は不変。tuning.generate.adjacentPosBoost（enabled:false で旧実装と
  bit一致＝300サンプル確認済みの避難路）。実測: SS起点の隣接検出比1.94:1（理論2:1と一致）・
  平均positionProf 隣接27.40 vs 非隣接25.26
- **GMボードのDH可視化＋並び順**（B-6）: GB_FIELD_POSITIONSにDHを追加し候補収集/飽和/ねじれ/
  弱点判定を他ポジと同型化（評価はwOBA百分位のみ＝守備不参照）。DH弱点時の「自軍の答え」は
  隣接縛りをバイパスし打撃百分位のみで候補化（守備出場ゼロの代打専任もヒット）。球団並びは
  自チーム先頭→自リーグ勝率降順→他リーグ（観測standingsのみ・同率id昇順の決定論）
- **DH運用の実態調査**（B-7前提・ユーザーの懐疑「本当にそんなことやってるの？」の裏取り）:
  結論「DH専任の育成・獲得戦略は実在しない」。NPBパは1球団3人以上分散が典型・MLBは専任
  400PA+が30球団中6のみで減少傾向・ドラフトのDH前提指名は確認できず・守備につけないことは
  市場で値下げ要因。本ゲームは位置調整-17.5と緩い回し運用が既に現実準拠で、唯一のギャップは
  「野手の休養日にDHへスライドさせる運用」の欠落 → B-7はこの1点のみ実装する方針に確定
  （thyroxin/research/dh_usage_research_20260725.md）

**課題点**: ①生成rng消費列の変更で正捕手先発101.5→99.2の玉突き→catcherRestProb 0.068→0.064
（0.060は過修正でHR王/xwOBA恒等を割る・中間値で61指標全green）②era.mjsのD3時代揺れテストが
ノイズ床ぎりぎりで符号反転→8シード平均へ頑健化 ③並行エージェントのフルテストが相手の編集
過渡状態を掴む事象（実害なし・領域分離は機能）

**ゲート**: npm test 733 PASS（+12新テスト）・calibrate 61指標 FAIL 0（帯変更なし）・
realism GATE 45/0・smoke 13フロー・verify identity

**残**: B-7の実装（休養日DHスライド・usage.mjs）

## 2026-07-25 (投手打席の廃止＝全リーグDH制へ統一・較正のパ・リーグ型再アンカー)

**やったこと**: ユーザー決定「投手が打席に立つのは廃止して全部DHに」。req_2②「セパ両リーグ近似」
からの**意図的乖離**（セ系の投手打席という現実の非対称性を捨てる）として記録の上、L1（陽炎）を
dh:true へ。エンジンのDH無し能力（buildDepthChart {dh:false}・game.mjsの経路）はテスト資産・
将来のルール変更オプションとして削除せず維持（リーグ設定のみの変更）。

- **較正ハーネス一般化**（tools/calibrate.mjs）: 旧実装は「DH無リーグが存在する」前提で全DH構成
  ではクラッシュ（noDhL=undefined参照）。セパ得点差はDH有/無の両方に試合がある場合のみ計算、
  犠打はリーグを一般走査して共通帯 shPerTeam で判定する形へ
- **概念が消滅した帯の削除**: runDiffDhMinusNoDh（セパ得点差[0.1,0.45]）・shPerTeamNoDh[55,110]
- **帯のパ・リーグ型再アンカー**（旧帯はセ(DH無)+パ(DH有)混合実測アンカーのため。全て根拠コメント付き）:
  OBP[0.32,0.328]→[0.322,0.332]・BB%[0.078,0.083]→[0.08,0.087]・ERA[3.5,3.9]→[3.6,4.0]・
  敬遠下限10→6（「8番を歩かせて投手勝負」の動機消滅）・Contact%[0.75,0.81]→[0.76,0.82]
  （低コンタクトの投手打者≈3%のPAが母集団から消えた組成効果＝野手挙動をノブで歪めず帯側で対応）
- **ノブ再較正（3巡で収束・全て文書化済みノブの前例幅）**:
  ①replHitterWinsPerTeamGame 0.129→0.142（L1にDHスロットが立ち負のDH位置調整が全リーグ化＋
    リーグ平均打撃上昇で野手wRAA圧縮→野手WAR比0.510→0.537へ復帰・総WAR416で帯内）
  ②smaxPerRating 0.018→0.017（母集団再構成の引き直しで|xwOBA−wOBA|=0.0034→0.0026へ・3回目の同機序）
  ③hrScale 0.9695→0.9655＋whiffZoneBase 0.1825→0.1845（②の守備上位圧縮で被安打微増→HR130.2/
    ERA4.07/R4.36の玉突きを相殺→最終 全帯内）
- **テスト反転**: 「投手打席はDH無しリーグ主催で発現」→「投手PAは両リーグとも厳密に0」の新不変量へ。
  スプリット恒等の標本数は野手のみ母集団（>200→>150・打席保持者が約180人=12球団×野手15人に）
- **シード感応テストの追随2件**: ①rSB×肩相関が3シード平均0.381と閾値0.4を微割れ→シード別実測
  （2026=0.647/2027=0.178/2028=0.318/2029=0.838/2030=0.665＝全シード正で機構健在）を記録し
  5シード平均（0.529）へ拡張・閾値0.4維持 ②多年ERA帯の上限4.78をyi19=4.80が0.02超過→
  1年目帯の再アンカーと同じ+0.1幅で4.9へ（多年再較正時に締め直すTODOは維持）

**課題点**: 較正3巡の玉突き（帯再アンカー→WAR比→守備スプレッド→HR/ERA）は各ノブの文書化済み
前例幅で機械的に収束できた。帯を緩めたのは「DH化の直接帰結」と説明できる5帯のみで、門番の
HR[110,130]と恒等|xwOBA−wOBA|≤0.003は帯を守ってノブで収束

**ゲート**: npm test 721 PASS・calibrate 61指標 FAIL 0（帯再構成で62→61）・realism GATE 45/0・
smoke 13フロー・verify identity

## 2026-07-24 (GM機能の品質改善: 3監査→GMボード判定修正＋スペクトラム跨ぎ昇格)

**やったこと**: ユーザー指摘「GM機能が雑（飽和がほぼ全部・救援の規定25%誤判定・二軍OPS10割超CF
×一軍LF弱点の放置）」を受け、3監査（GMボード判定/起用AI昇格経路/複数ポジ適性の現実調査）→
修正2ウェーブ（Sonnet委任→レビュー→ゲート）。

- **監査結果（数字つき）**: ①飽和★が120セル中87.5〜90%に点灯し指標として死んでいた。主因は
  「控えの観測がレギュラーを上回る逆転」が79%（多面起用選手が控え枠に回っただけの誤検出）＋
  同一選手の複数ポジ多重カウント19-24%＋救援を先発基準の規定投球回で測る誤り。
  ②昇格経路が全て primaryPos 完全一致ゲート＝隣接ポジの当たり選手が候補に入らず、60チーム年
  中38でミスマッチ（例: CF余り×LF穴の放置）。③現実調査: SS経験者の3B兼任は極めて一般的
  （守備スペクトラム）→ thyroxin/research/position_versatility_research_20260724.md に3案
- **GMボード修正**: saturated（真のサプラス）と misallocated（起用のねじれ=逆転・トレード材料に
  しない）を分離（satMinPctl 0.6→0.8・satMisallocMargin 0.55は7seed実測で飽和13〜23%=目標帯
  15-25%に収束させて選定）。多重カウント排除（他ポジregular除外・最多出場1ポジ限定）。救援の
  役割別分母 gbQualifiedReliefIP（relieverIpGamesFrac 0.45・25登板以上は「塞がれ」対象外）。
  「自軍の答え」ownDepthSolutions（弱点×自軍二軍/控えの高百分位×隣接ポジ）を弱点表の下に表示
- **スペクトラム跨ぎ昇格（案B）**: positions.mjs に spectrumDistance（SS-2B-3B・LF-CF-RF・
  1B-3B/1B-LF/1B-RF橋・C/DHは隔絶）。昇降格の適格判定を完全一致→距離1以内へ拡張
  （crossPosPromotion・falseで旧挙動の避難路）。跨ぎ候補は対象ポジの観測守備適性
  （obsTool=スカウトノイズ経由・三層構造準拠）が crossPosFitRef を下回るぶん減点＝
  「守れなくはないが本職ではない」の一段落ちで同ポジ候補との不等式を成立させる

**課題点（2件・原因と対処を記録）**:
- 較正玉突き: ロスター構成が変わり故障件数比0.849と帯下端0.85を微割れ→pitcherMult 1.35→1.38
  の1ノッチで1.10（帯中央）へ再収束
- F2-2不変量「二軍の各主ポジに farmKeepPerPos 人残置」が跨ぎ昇格で崩れた（T1の二軍3Bが枯渇）
  → farmCandidates に残置ガード（跨ぎ候補は引き抜き後も当該ポジに規定人数残る場合のみ許容）。
  同ポジ入替は1:1補充で人数不変・IL復帰は補充時と対で相殺・育成⇄支配下は二軍人数不変のため
  ガードはこの1点で全経路を覆う

**ゲート**: npm test 721 PASS・calibrate 62/62・realism GATE 45/0（故障件数比1.10）・
smoke 13フロー・verify identity

**残課題**: 案C（生成時の隣接ポジ優先ブースト）・GMボードのDH不可視と球団並び順・
アナリストコラム定常供給（P2）

## 2026-07-24 (レビュー対応Wave B: 情報設計一式 — P0-2/P0-3/P1-6/P1-8)

**やったこと**: 第1回フルレビューの情報設計指摘への対応（Sonnet委任→レビュー→ゲート）。
- P0-2 記録タブのサブタブ化（観点10）: 6セクション縦積み12.87画面→サブタブ分割
- P0-3 成績タブの行ページング（観点10）: `paginate`純関数（上位30行＋さらに表示・ソート/列
  グループ変更でリセット）をstatTable/WARリスト共通適用。100行一括3.6-4.2画面を解消
- P1-6 観戦「指標の変化」のサンプルゲート（観点11）: 極小サンプルは「参考値までもう少し」表記
  （※小サンプルの極端値=ERA27.00やkwERA負値は正当な値というユーザー指摘を反映し、隠すのでなく
  枠づける方針）
- P1-8 チーム選択カードに特色1行（観点5）: パーク傾向/監督タイプ/資金力（公開情報のみ）

**良かった点**: レビューP0/P1の8指摘中7件をWave A/Bの2巡で解消（残: P2軽微3件）
**課題点**: ページング導入で既存smokeアサーション（役割セル両方出現）が上位30行前提と衝突
→「さらに表示」全展開後に検証する形へ追随（表示分割系の変更は既存アサーションの全件洗いが必要）

**ゲート**: npm test 全PASS・smoke 13フロー・verify identity・calibrate 62/62

## 2026-07-24 (レビュー対応Wave A: 介入UX一式 — P0-1/P1-4/P1-5)

**やったこと**: 第1回フルレビューの介入系指摘への対応（Sonnet委任→レビュー→較正）。
- P0-1 介入モーダルの判断材料（観点5・P1仕様§4の完成）: 代打候補=当季成績＋対左右プラトーン＋
  フォーム▲▼（form.mjs流用・成績順ソート）、継投候補=防御率/K-BB%＋役割タグ＋連投状態（防御率順）。
  現打者/現投手にも同情報。sim側は situ.oppPitcherId（表示専用）1フィールドのみ追加＝乱数非消費
- P1-4 特別デー「首位攻防」の希少化（観点14）: 判定を「首位との差」基準へ変更＋12seed sweepで
  0.5差×6割消化=月2.8回に較正（旧: 相手直接差1.5で試合日30.8%発火）。終盤の優勝争い限定の
  意味論に収束
- P1-5 WPAカード×介入ログの⚡タグ（観点6/8）: choice.pick一致＋box証跡（代打='打'スロット/
  救援=非先発）の二重裏取りで「⚡采配直後」表示。曖昧なら付けない

**良かった点（数字つき）**: 全おまかせbit同一テスト維持のままモーダル情報量を6項目追加・
発火頻度を30.8%→13.2%（月2.8回）へ較正・test 686 PASS(+20)

**課題点**: 閾値変更でQ3テストフィクスチャ1件が旧閾値前提だった（修正済み・閾値変更時は
フィクスチャの前提値も同時に見る）

**ゲート**: npm test 686 PASS・smoke 13フロー・verify identity・calibrate 62/62・realism GATE 45/0

## 2026-07-24 (第1回ゲーム開発フルレビュー — /game-review full・観察3系統並列)

**やったこと**: game-reviewスキル初回実行。①初見の肩越し観察（リポジトリ知識ゼロのエージェントが
実プレイ・スクショ15枚）②情報設計390px実測（全タブscrollHeight・25枚）③意思決定/手触り/長期
ループ検査（コード＋calibrate分布＋1シーズン発火頻度実測）。詳細: thyroxin/reviews/game_review_20260724.md

**良かった点（数字つき）**:
- 初見7手・約2-3分で核体験（継投指示）到達＝観点3合格。試合後ニュース生成が初見の惹き1位
- 前回UI/UXレビュー指摘の改善実証: 日程タブ5080→1592px（-69%）・ホーム1.31画面・順位0.95画面
- 采配発現帯すべて帯中位（犠打78.4/41.6・敬遠14.0）＝ドミナント戦略なし・トレード/育成の
  意思決定設計は模範例判定

**課題点（数字つき）**:
- 【P0】介入モーダルに候補の判断材料ゼロ（P1仕様§4の「成績/対左右/役割タグ」が実装欠落・
  初見と構造検査が独立に最重大指摘）
- 【P0】記録タブ12.87画面・成績タブ100行一括3.6-4.2画面（前回から未改善）
- 【P1】特別デー「首位攻防」が試合日の30.8%で発火（希少性崩壊・判定式が首位でなく直接差）・
  WPAカード×介入ログ未接続・ERA -27.00のありえない表示（要調査バグ）

**次にやること**: レビューの次アクション候補1-5をユーザーと確認して実装へ。

## 2026-07-24 (RE駆動の戦術判断＋走塁指標UI — ユーザー指示「得点期待値で比較。走塁指標が出てない」)

**RE戦術（sim側・Sonnet委任→レビュー）**:
- 「監督の頭の中のRE表」`tuning.tactics.reTable`（24状態）を**本ゲームの参照環境から実測導出**
  （自己参照の不安定性=RE表が采配を変え采配がRE表を変える、を8シード平均×3回反復で収束・
  残差<0.15・導出スニペットをconfigコメントに記録）。盗塁損益分岐0.71-0.75が既存設計値
  steal.successBase=0.72と一致＝表の妥当性の外部検証
- バント: `buntDecisionScore` = ΔRE(成功/失敗/内野安打×resolveBunt実遷移) − ΔRE(強攻=打者wOBA
  のrun換算) ＋ 次打者項（nextIsPitcherは観測wOBA経由で自然に大減点）。試行確率 =
  expit(reScale×score + npbBias + buntTend)。npbBias=NPBの送りバント文化の明示ノブ（原則②）
- 盗塁: 損益分岐サニティ（推定成功率がbreakevenを大きく下回るとロジット減点・発現帯は不変）
- 敬遠: ΔRE損の上限ゲート（実データでは発火しない純安全網=0.30>実測最大0.238）
- dceb1a9の2保険ゲートは維持（RE単独では当該局面が18-19%残る実測に基づく判断）
- 較正: reScale=4.4/npbBias=0.75/nextBatterW=0.55 の初期値で62/62一発収束。era.testの
  単一シードサニティ（rpw差>0.2がseed11のみ0.156）は3シード平均化（ARM相関と同じ前例・
  stashでベースライン無罪を検証済み）

**走塁指標UI（表示層・Sonnet委任・並行）**:
- 成績ビュー打撃タブに「走塁」列グループ（SB/CS/盗塁%/XBT%/BsR・ヒート付き・機会0は'-'）
- 選手モーダル基本タブに走塁節・年度別成績にBsR列（二軍行はlc水準不整合のため'-'）・
  チームタブにBsR列。ツールチップはXBT%=追加進塁率(advTaken/advOpp)等の正確な定義

**ゲート**: npm test 666 PASS（+10: tactics_re 9本等）・build・smoke 13フロー・verify identity・
calibrate 62/62・realism GATE 45/0 全PASS。（途中コンテナ再起動でゲート再実行1回）

## 2026-07-23 (采配妥当性: バント2ゲート — ユーザー指摘「一死二塁で投手の前の打者が送りバント」)

**きっかけ**: ユーザーの実プレイ指摘。二重の悪手＝①一死二塁からの送りバントは成功しても二死三塁
（RE24 ≈0.68→0.35）で犠飛も使えずほぼ常に損 ②送っても次打者が投手では敬遠or凡退がオチ。

**修正**（manager.mjs buntAttemptProb・原則②「起用/采配の妥当性」）:
- **一死×二塁単独ゲート**: 野手には送らせない（無死二塁→一死三塁は犠飛圏なので従来どおり許可。
  投手打席も従来どおり＝打たせても期待値が低く実NPBでも見る）
- **次打者投手ゲート**: 野手には送らせない（「投手の前の8番に送らせる」は実NPBで批判される典型
  悪手。送るなら9番=投手自身の打席）。呼び出し側で計算済みだった nextIsPitcher を situ へ配線
- テスト追記（manager.test.mjs 14 PASS・新ゲート4アサーション）

**較正トレッドミル（乱数消費変更＝引き直し・3周で収束）**:
1. |xwOBA−wOBA| 0.0033>帯0.003 → smaxPerRating 0.019→0.018（文書化済みノブの1ノッチ・前例踏襲）
2. Contact% 0.810微超過 → whiffZoneBase 0.181→0.1825（同・苗字プール拡張時と同じ相殺）
3. ARM×肩相関 seed2026=0.489<0.5 の下振れ → ブロッキング/rSBと同じ**3シード平均化**
   （実測 2026=0.489/2027=0.579/2028=0.465＝機構健在・閾値0.45）

**ゲート**: manager 14 PASS・phaseB_b3b 11 PASS・calibrate 62/62（犠打帯: セ系70.0/パ系30.2 PASS）・
realism GATE 45/0・smoke 13フロー・verify identity 全PASS（フルテストは前回656/1→FAIL分を修正）。
