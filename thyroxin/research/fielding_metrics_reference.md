...
## 14. ポジション別の走力・肩（生成器の事前分布）

2026-07-09 の追加調査。**「Spd の守備位置速度って本当にいいの？」というユーザーの問いから始まり、
生成器そのものがポジション別の走力・肩を二値スイッチで与えていたことが判明した。**
...
const arm   = draw(rng, primaryPos === 'RF' || primaryPos === 'C'  ? 58 : 50, 10);
const steal = draw(rng, speed > 55 ? 55 : 46, 12);
```
- 走力: CF/SS だけ +10、残り6ポジションは**全員フラット**（捕手が遅くない・二塁手が速くない）
- 肩: RF/C だけ +8、残りは**全員同じ**（一塁手と遊撃手の肩が同じ）
- 盗塁技術: **speed 55 で 46→55 と 9pt 跳ぶ階段関数**。speed 25 の選手と speed 54 の選手の盗塁技術が同じ

### 14.2 Sprint Speed（Baseball Savant 2024・CSVから自己集計） `[一次]`

`https://baseballsavant.mlb.com/leaderboard/sprint_speed?year=2024&min=10&csv=true`
（競争的走塁10回以上・N=566）。**集計したリーグ全体平均 27.30 ft/sec が公称 27 ft/sec と一致**＝検算OK。

| pos | n | 平均 (ft/sec) | 全体差 | 位置内SD |
|---|---|---|---|---|
| CF | 56 | **28.68** | +1.38 | 0.83 |
| SS | 58 | 27.93 | +0.62 | 1.15 |
| LF | 61 | 27.87 | +0.57 | 0.97 |
| RF | 67 | 27.79 | +0.49 | 0.90 |
| 2B | 64 | 27.61 | +0.31 | 1.12 |
| 3B | 81 | 27.30 | −0.01 | 0.95 |
| 1B | 56 | 26.32 | −0.98 | 1.03 |
| C | 79 | **25.97** | −1.33 | 1.27 |

- **選手個人のSD = 1.36 ft/sec**（2023: 1.42 / 2025: 1.35 と3年安定）
- 序列: **CF > SS ≈ LF ≈ RF > 2B > 3B > 1B > C**（3年連続で同順）
- **捕手が最も遅い**（3年とも最下位）。**二塁手は三塁手より速い**（3年連続 +0.2〜0.5 ft/sec）
- ポジション間スプレッド = 2.71 ft/sec ≈ 個人SDの **2.0σ**
- 参考: FanGraphs "Why Are Catchers So Dang Slow?" — 21歳時点で全体28.2 vs 捕手25.5 ft/sec

### 14.3 Arm Strength（Baseball Savant 2024・公表のポジション別平均） `[一次]`

`https://baseballsavant.mlb.com/leaderboard/arm-strength?year=2024`（上位10%送球の平均・2020年〜Hawk-Eye）
...
- 序列: **CF ≈ RF > LF > SS > 3B >> 2B > 1B**。「捕手と右翼手だけ強肩」は**明確に誤り**
- 内野は 1B/2B が 3B/SS より **6〜9 mph も弱い**
- 選手個人の pooled SD ≈ **6 mph**（`arm_overall` 5.77 / `max_arm_strength` 6.62 で挟んだ自己集計値）
- 位置内SD ≈ 4〜5 mph（1Bのみ 6.9＝打撃で残る転向組が混じる）
- **⚠ 捕手は Savant が Arm Strength から除外している**（Pop Time で評価する。MLB平均 2.0秒）。
  → **捕手の平均 mph は一次情報に存在しない。** 2017年のトップ捕手の max-effort が 87〜88 mph
  （＝外野平均と同水準）という二次情報のみ。実装では RF と同値を**設計値**として置き、明示した。

### 14.4 レーティングへの写像

**1 rating pt = 選手個人の標準偏差の 0.1**（＝σ = 10 rating）と定める。

```
走力 bias = (ポジション平均 − 27.30) / 1.36 × 10
肩   bias = (ポジション平均 − 85.17) / 6.00 × 10
```

`draw` の sd は、独立な2つの制約が一致した:

| | 母集団sdを現行と一致させる値 | 実測の位置内SD（rating換算） | 採用 |
|---|---|---|---|
| 走力 | 9.97 | 6.1〜9.3 | **10** |
| 肩 | 7.06 | 6.7〜8.3 | **7.1** |

base は生成人数で重み付けした平均が現行値（走力 50.70 / 肩 52.44）になるよう定めた（較正の揺れを最小化）。

盗塁技術は走力と連続に結線した。傾き（0.2636）・切片・残差sd は**旧実装の周辺分布**
（平均 48.85 / sd 12.56 / steal〜speed の回帰係数）を保つよう定めた ＝ **段差の撤廃だけを行い、新しい数値を発明していない。**
...
母集団: speed 50.61±11.59（旧 50.70±11.77）/ arm 52.75±10.10（旧 52.44±10.27）/ steal 48.63±12.47（旧 48.85±12.56）

**副作用**: 捕手・一塁手が現実どおり遅くなったため、余分な塁を取らなくなり得点が 4.20 → 4.09 R/G へ下がった
（率系 AVG/OBP/SLG はほぼ不変＝安打ではなく走塁の変化）。得点環境中立を保つため `field.smaxBase` を
6.82 → 6.78 へ再較正した。較正62指標すべて PASS。

**⚠ 記録: 総WAR が 429（上限430）と際どい。** 得点環境が下がると `rpw`（1勝あたり得点）が下がり、
代替水準が **run で定義されている**ため総WARが上がる。本来 replacement level は win で定義すべきで、
これは構造的な問題である（本作業の範囲外）。

---

## 15. Spd（Speed Score）の撤去

`[一次]` FanGraphs Library の Spd ページ:
- FanGraphs 版は **4成分の平均**（盗塁成功率 / 盗塁企図頻度 / 三塁打率 / 得点率）。**守備位置の項は含まない**
...
- **各 factor の正確な係数はページに記載が無く、一次情報で確認できなかった**
  （Wikipedia 単独ソースで、しかも一般に流布する式と係数が食い違う: F3 の係数 625 vs 500、F4 の分母定義）

旧実装の問題:
1. **誰も使っていなかった**（`playerBaserunning()` が返すだけ。UI・起用AI・較正のいずれも参照せず）
2. `守備位置速度` は**打席を1つも見ないルックアップ項**で、真値speed と r=0.29 で相関した（＝作った相関）
3. しかもその表はこのシムの生成分布と矛盾していた（C=2 だが実際は最遅ではなかった／2B=6 だが速くなかった）
4. FanGraphs の4成分目「得点率」は、打者の得点(R)カウンタが statline に存在せず**計算できない**

→ **削除した。** 走塁の価値は BsR = UBR + wSB + wGDP（すべて一次情報で定義され、生カウントから創発）で表す。
これは req_20260708 line9「リアリティを損なうものがあればもう出さなくてもいい」に従う判断でもある。
